...
public class ManHinhChinh {
    ...
    // Sidebar Buttons
    @FXML
    private Button manHinhChinhButton;
    @FXML
    private Button quanLyDatBanButton;
    @FXML
    private Button quanLyThongKeButton;
    @FXML
    private Button quanLyThucDonButton;
    @FXML
    private Button quanLyHoaDonButton;
    @FXML
    private Button quanLyNhanVienButton;
    @FXML
    private Button quanLyKhachHangButton;
    @FXML
    private Button quanLyKhuyenMaiButton;
    @FXML
    private Button quanLyTraCuuButton;
    @FXML
    private Button dangXuatButton;
    // Sidebar Icons
    @FXML
    private ImageView manHinhChinhIcon;
    @FXML
    private ImageView quanLyDatBanIcon;
    @FXML
    private ImageView quanLyThongKeIcon;
    @FXML
    private ImageView quanLyThucDonIcon;
    @FXML
    private ImageView quanLyHoaDonIcon;
    @FXML
    private ImageView quanLyNhanVienIcon;
    @FXML
    private ImageView quanLyKhachHangIcon;
    @FXML
    private ImageView quanLyKhuyenMaiIcon;
    @FXML
    private ImageView quanLyTraCuuIcon;
    @FXML
    private ImageView dangXuatIcon;
    // TopBar & Theme
    @FXML
    private HBox topBar;
    @FXML
    ...
    // Menu Items
    @FXML
    private MenuItem doiMatKhauMenuItem;
    @FXML
    private MenuItem caiDatMenuItem;
    @FXML
    private MenuItem toggleThemeMenuItem;
    @FXML
    private MenuItem dangXuatMenuItem;
    @FXML
    ...
    public void initialize() {
        ...
        // Map Icons - Hãy đảm bảo tên file trong src/main/resources/icons khớp với tên
        // ở đây
    ...
    public void chuyenSangTabDatBan(HoaDon hd) {
        try {
            ...
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Lỗi chuyển tab",
                    "Không thể chuyển sang màn hình Đặt bàn: " + e.getMessage());
        }
    }
    ...
    private void apDungPhanQuyen() {
        ...
        if (guiChuongTrinhTVMenuItem != null)
            guiChuongTrinhTVMenuItem.setDisable(!vaiTro.coQuyenQuanLyKhachHang());
        if (xemLogKiemKeTienMatMenuItem != null)
            xemLogKiemKeTienMatMenuItem.setDisable(!vaiTro.coQuyenThongKe());
    }
    ...
    private void loadScreen(String fxmlPath, String cssPath) throws IOException {
        // 1. Dọn dẹp màn hình cũ (Hiệu năng - Nhánh lechung)
        if (currentController instanceof RealTimeSubscriber) {
            ((RealTimeSubscriber) currentController).cleanupRealTime();
        }

        // 2. Kiểm tra Cache (Hiệu năng - Nhánh lechung: Không cache DatBan và TraCuu)
        ...
        if (!skipCache && screenCache.containsKey(fxmlPath)) {
            ...
            // 🔥 Cập nhật ca trực & dữ liệu khi quay lại Dashboard (Nhánh phuocmai)
    ...
    private void handleQuanLyThucDon() throws IOException {
        if (currentUser == null || currentUser.getVaiTro() == null
                || !kiemTraQuyen("Quản lý Thực đơn", currentUser.getVaiTro().coQuyenQuanLyThucDon()))
            return;
    ...
    private void handleQuanLyHoaDon() throws IOException {
        if (currentUser == null || currentUser.getVaiTro() == null
                || !kiemTraQuyen("Quản lý Hóa đơn", currentUser.getVaiTro().coQuyenQuanLyHoaDon()))
            return;
    ...
    private void handleQuanLyNhanVien() throws IOException {
        if (currentUser == null || currentUser.getVaiTro() == null
                || !kiemTraQuyen("Quản lý Nhân viên", currentUser.getVaiTro().coQuyenQuanLyNhanVien()))
            return;
    ...
    private void handleQuanLyKhachHang() throws IOException {
        if (currentUser == null
                || !kiemTraQuyen("Quản lý Khách hàng", currentUser.getVaiTro().coQuyenQuanLyKhachHang()))
            return;
    ...
    private Optional<Pair<String, String>> showChangePasswordDialog() {
        ...
        grid.add(new Label("Xác nhận MK mới:"), 0, 2);
    ...
    private void handleGuiChuongTrinhTV() {
        ...
        if (dsKhachHangCoEmail.isEmpty()) {
            showAlert(Alert.AlertType.INFORMATION, "Thông báo", "Không tìm thấy khách hàng nào có địa chỉ email.");
            return;
        }
        ...
        recipientListView.setCellFactory(lv -> new ListCell<KhachHang>() {
            ...
            protected void updateItem(KhachHang kh, boolean empty) {
                ...
                if (empty || kh == null) {
                    ...
                    checkBox.setSelected(selectionMap.get(kh).get());
        ...
        selectAllCheckBox.setOnAction(e -> {
            selectionMap.values().forEach(prop -> prop.set(selectAllCheckBox.isSelected()));
        ...
        grid.add(recipientListView, 1, 0);
        grid.add(selectAllCheckBox, 1, 1);
        grid.add(new Label("Tiêu đề:"), 0, 2);
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == sendButtonType) {
                ...
                res.put("recipients", selectionMap.entrySet().stream().filter(en -> en.getValue().get())
                        .map(en -> en.getKey().getEmail()).collect(Collectors.toList()));
        ...
        if (result.isPresent()) {
            ...
            if (recipients.isEmpty()) {
                showAlert(Alert.AlertType.WARNING, "Thông báo", "Bạn chưa chọn khách hàng nào để gửi email.");
                return;
            }
            if (subject.trim().isEmpty() || bodyContent.trim().isEmpty()) {
                showAlert(Alert.AlertType.WARNING, "Thiếu thông tin", "Vui lòng nhập Tiêu đề và Nội dung email.");
                return;
            }
            ...
            new Thread(() -> {
                try {
                    ...
                    Platform.runLater(() -> showAlert(Alert.AlertType.INFORMATION, "Thành công",
                            "Đã gửi email đến " + recipients.size() + " khách hàng."));
                } catch (MessagingException e) {
                    Platform.runLater(() -> showAlert(Alert.AlertType.ERROR, "Lỗi Gửi Email",
    ...
    private void saveKiemKeLog(double heThong, double thucTe) {
        ...
        try (java.io.BufferedWriter writer = new java.io.BufferedWriter(
                new java.io.FileWriter("nhat_ky_kiem_ke.txt", true))) {
        ...
        try {
            ...
            if (url != null) {
                ...
                webEngine.loadContent(
                        "<h1 style='color:white; background:#0f172a; height:100vh; display:flex; align-items:center; justify-content:center;'>Lỗi: Không tìm thấy file HTML!</h1>");
            }
            Scene scene = new Scene(webView);
            stage.setScene(scene);
