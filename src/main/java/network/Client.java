...
* Mỗi yêu cầu mở một kết nối mới (Short-lived connection) để đảm bảo tính ổn định.
*/
public class Client {
    ...
    */
    ...
    private static Response sendRequest(Request req) {
        System.out.println("[API] Sending request: " + req.getAction() + (req.getToken() != null ? " (with token)" : ""));
        ...
        return new Response(503, "Không thể kết nối đến máy chủ. Vui lòng kiểm tra lại mạng hoặc trạng thái Server.", null);
    } catch (Exception e) {
