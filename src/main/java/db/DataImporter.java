...
if (i + 1 < content.length() && content.charAt(i+1) == ' ') i++;
} else {
sb.append(c);
}
}
...
private String clean(String val) {
    ...
    if (val.startsWith("N'")) val = val.substring(2, val.length() - 1);
    else if (val.startsWith("'")) val = val.substring(1, val.length() - 1);
...
private void parseBan(String line) {
    ...
    if (loai != null) b.setLoaiBan(LoaiBan.valueOf(loai));
    String tt = clean(vals.get(4));
    if (tt != null) b.setTrangThai(TrangThaiBan.fromDbValue(tt));
    tableDAO.insert(b);
}
...
private void saveEmployees() {
    for (NhanVien nv : employees.values()) {
        ...
        if (role.equalsIgnoreCase("Admin") || role.equalsIgnoreCase("QuanLy")) nv.setChucVu("QuanLy");
        else nv.setChucVu("NhanVien");
...
private void parseUuDai(String line) {
    ...
    if (ns != null) ud.setNgayBatDau(LocalDate.parse(ns));
    String ne = clean(vals.get(5));
    if (ne != null) ud.setNgayKetThuc(LocalDate.parse(ne));
    promoDAO.insert(ud);
}
private void parseKhachHang(String line) {
    ...
    if (ndk != null) kh.setNgayDangKy(LocalDate.parse(ndk));
...
private void parseHoaDon(String line) {
    ...
    if (nl != null) hd.setNgayLap(LocalDateTime.parse(nl));
    String pt = clean(vals.get(3));
    if (pt != null) {
        if (pt.equalsIgnoreCase("ViDienTu")) hd.setHinhThucTT(PTTThanhToan.VI_DIEN_TU);
        else if (pt.equalsIgnoreCase("NganHang")) hd.setHinhThucTT(PTTThanhToan.NGAN_HANG);
        else hd.setHinhThucTT(PTTThanhToan.TIEN_MAT);
    }
    ...
    if (tc != null) hd.setTienCoc(Double.parseDouble(tc));
    String gv = clean(vals.get(5));
    if (gv != null) hd.setGioVao(LocalDateTime.parse(gv));
    String gr = clean(vals.get(6));
    if (gr != null) hd.setGioRa(LocalDateTime.parse(gr));
